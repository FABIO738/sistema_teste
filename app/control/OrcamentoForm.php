 ...
 */
class OrcamentoForm extends TWindow
{
    protected $form; // form
    ...
    function __construct()
    {
        ...
        // creates the form
        $this->form = new BootstrapFormBuilder('form_orcamento');
        ...
        $produto_detail_produto_id  = new TDBUniqueSearch('produto_detail_produto_id', 'curso', 'ProdutoOrcamento', 'id', 'descricao');
        ...
        $this->form->addContent( ['<h4>Venda detalhada</h4><hr>'] );
        ...
        $this->form->addFields( [ new TLabel('Preço (*)', '#FF0000') ],   [$produto_detail_preco_venda],
                                [ new TLabel('Desconto')],                [$produto_detail_desconto] );
        
        $add_product = TButton::create('add_product', [$this, 'onProdutoAdd'], 'Registrar', 'fa:plus-circle green');
        ...
        $col_preco_venda  = new TDataGridColumn( 'preco_venda', 'Preço', 'right', '15%');
        ...
        $col_descr->setTransformer(function($value) {
            return ProdutoOrcamento::findInTransaction('curso', $value)->descricao;
        });
    ...
    public static function onProdutoChange( $params )
    {
        if( !empty($params['produto_detail_produto_id']) )
        {
            try
            {
                ...
                $product   = new ProdutoOrcamento($params['produto_detail_produto_id']);
                TForm::sendData('form_orcamento', (object) ['produto_detail_preco_venda' => $product->preco_venda ]);
                TTransaction::close();
            }
            catch (Exception $e)
            {
    ...
    public function onProdutoAdd( $param )
    {
        try
        {
            ...
            TForm::sendData( 'form_orcamento', $data, false, false );
        }
        catch (Exception $e)
        {
    ...
    public function onSave($param)
    {
        try
        {
            ...
            //OrcamentoItem::where('orcamento_id', '=', $sale->id)->delete();
            ...
            TForm::sendData('form_orcamento', (object) ['id' => $sale->id]);
    ...
    public function onEdit($param)
    {
        try
        {
            ...
            if (isset($param['key']))
            {
                ...
                $venda_items = OrcamentoItem::where('orcamento_id', '=', $object->id)->load();

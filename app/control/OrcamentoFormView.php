...
class OrcamentoFormView extends TPage
{
    ...
    public function __construct($param)
    {
        ...
        $this->form->setFormTitle('FALCÃO PNEUS <BR>CNPJ: 30.228.240/0001-09<br>FONE: (66)3423-9966  (66)9 9620-4109<BR><br>Orçamento');
        
        $this->form->setColumnClasses( 2, ['col-sm-3', 'col-sm-7'] );
    ...
    public function onView($param)
    {
        try
        {
            ...
            $venda = new Orcamento( $param['key'] );
            
            $this->form->addFields( [ new TLabel('Código: ')], [ new TTextDisplay( $venda->id, '#333'), ] );
            $this->form->addFields( [ new TLabel('Data: ')], [ new TTextDisplay( $venda->dt_orcamento, '#333') ] );
            ...
            $this->form->addFields( [ new TLabel('Observação: ')], [ new TTextDisplay( $venda->obs, '#333') ] );
            
            $list = new BootstrapDatagridWrapper( new TDataGrid);
            $list->style = 'width:100%';
            
            $col_produto  = new TDataGridColumn('produto_orcamento->descricao', 'Produto', 'left');
            ...
            $itens = OrcamentoItem::where('orcamento_id', '=', $venda->id)->load();
    ...
    {
        unset($param['static']);
        AdiantiCoreApplication::loadPage('OrcamentoForm', 'onEdit', $param);
    }
